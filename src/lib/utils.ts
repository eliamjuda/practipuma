...
export const getSubjectByName = (name: string): Subject | undefined => {
  return SUBJECTS_CATALOG.find(subject => subject.name.toLowerCase() === name.toLowerCase());
};

export const getSubtopicById = (subjectId: number, subtopicId: number): Subtopic | undefined => {
...
export const searchSubtopics = (query: string): Array<{subject: Subject, subtopic: Subtopic}> => {
  const lowerQuery = query.toLowerCase();
